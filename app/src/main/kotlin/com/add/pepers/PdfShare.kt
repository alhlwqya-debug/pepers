...
internal fun shareWebViewAsPdf(
    context: Context, webView: WebView, jobName: String, phone: String, email: String, onFinished: () -> Unit
) {
    try {
        ...
        val attributes = PrintAttributes.Builder()
            .setMediaSize(if (jobName.contains("فردي")) PrintAttributes.MediaSize.ISO_A4.asLandscape() else PrintAttributes.MediaSize.ISO_A4)
            .setMinMargins(PrintAttributes.Margins.NO_MARGINS)
            .setColorMode(PrintAttributes.COLOR_MODE_COLOR).build()
        val adapter = webView.createPrintDocumentAdapter(jobName)
        PdfPrint(attributes).print(adapter, pdfFile, object : PdfPrint.Callback {
            override fun onSuccess(file: File) {
                try {
                    ...
                    context.startActivity(Intent.createChooser(send, "مشاركة تقرير PDF"))
                    Toast.makeText(context, "تم إنشاء ملف PDF وفتح قائمة المشاركة.", Toast.LENGTH_SHORT).show()
                } catch (e: Exception) {
