...
internal fun AppLockScreen(context: Context, onUnlocked: () -> Unit, onUseDeviceLock: () -> Unit) {
    ...
    Column(
        ...
        Column(horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            Text("Pepers", fontSize = 23.sp, fontWeight = FontWeight.Bold, color = Purple)
        ...
        Column(
            ...
            OutlinedTextField(
                value = pin,
                onValueChange = { pin = it.filter(Char::isDigit).take(6); error = "" },
                label = { Text("رمز PIN") },
                singleLine = true,
            ...
            Spacer(Modifier.size(12.dp))
            Button(
                onClick = { if (verifyAppPin(context, pin)) onUnlocked() else error = "رمز PIN غير صحيح" },
    ...
    AlertDialog(
        ...
        text = {
            Column {
                ...
                OutlinedTextField(first, { first = it.filter(Char::isDigit).take(6) }, label = { Text("الرمز الجديد") }, singleLine = true, keyboardOptions = KeyboardOptions(keyboardType = KeyboardType.NumberPassword), modifier = Modifier.fillMaxWidth())
                Spacer(Modifier.size(7.dp))
        ...
        confirmButton = {
            Button(onClick = {
                when {
                    first.length !in 4..6 -> error = "الرمز يجب أن يكون من 4 إلى 6 أرقام"
            ...
            Column(verticalArrangement = Arrangement.spacedBy(9.dp)) {
                ...
                Card(
                    ...
                    Row(Modifier.padding(13.dp), verticalAlignment = Alignment.CenterVertically) {
                        Icon(Icons.Default.Backup, null, tint = Green, modifier = Modifier.size(24.dp))
                        Spacer(Modifier.width(9.dp))
                        Column(Modifier.weight(1f)) {
                            Text("النسخ الاحتياطي والاستعادة", fontWeight = FontWeight.Bold)
                            Text("حفظ نسخة من بيانات المحل واستعادتها عند الحاجة.", fontSize = 10.sp, color = Color.Gray)
                        }
                    }
                }
            }
                ...
                Card(
                    ...
                    Column(Modifier.padding(14.dp)) {
                        Row(verticalAlignment = Alignment.CenterVertically) {
                            ...
                            Spacer(Modifier.width(8.dp))
                            Text("نسخة بياناتك", fontWeight = FontWeight.Bold)
                        }
                        Spacer(Modifier.size(6.dp))
...
internal fun restoreBackup(context: Context, uri: Uri): Boolean {
    return try {
        ...
        if (imageSource.exists()) copyFile(imageSource, File(context.filesDir, "profile_image.jpg"))
